...
export class PayPage {
  ...
  constructor(private iab: InAppBrowser, public uM: UserManager, public nS: NativeStorage, public tC: ToastController, public nP: NavParams) {
  ...
  pay() {
    let desc = "Финансовая подждержка бойца: " + this.selectedUser.surname + " " + this.selectedUser.name;
    this.uM.addPay(this.user.id, desc).subscribe((data: any) => {
      let result = JSON.parse(data['_body']);
      const browser = this.iab.create('https://sci.interkassa.com/?ik_co_id=5a72c6683b1eaf18508b456d&ik_pm_no='+result.pay_id+'&ik_am='+this.payCount+'&ik_cur=RUB&ik_desc='+desc+'&ik_exp=2018-02-02&ik_x_type=1&ik_x_ids='+this.selectedUser.id);
